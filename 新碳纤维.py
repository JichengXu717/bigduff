...
def kfold_cv_details(estimator, X_data, y_data, cv_splits=5):
...
for target_name, X_train, X_test, y_train, y_test, y_full in [
    ...
    for model_name, model_info in models.items():
        ...
        cv_df = kfold_cv_details(best_model, X, y_full, cv_splits=5)
        cv_df.insert(0, 'Model', model_name)  # 加一列模型名
...
for target_name, y_full in [
    ...
    for model_name, model_est in results[target_name]['final_model'].items():
        ...
        cv_for_curve = KFold(n_splits=5, shuffle=True, random_state=42)
        ...
        plt.plot(train_sizes, train_scores_mean, marker='o', label='Training R$^2$')
        plt.fill_between(
            train_sizes,
        ...
        plt.plot(train_sizes, val_scores_mean, marker='o', label='Validation R$^2$')
        plt.fill_between(
            train_sizes,
